...
public class Game implements Observable {
    ...
    public Player logIn(String name){
        ...
        Snake sn = new Snake(getCoordinate(Status.SNAKE));
        userList.put(pl,sn);
        makeFruit();
        return pl;
    }
    ...
    public void makeFruit(){
        while((field.numOfEmptyElField() > 0) && (fruit.size() < userList.size())) {
    ...
    public void delFruit(Cell head){
        for(int i = 0; i < fruit.size(); ++i){
            if ((head.getX() == fruit.get(i).getX()) && (head.getY() == fruit.get(i).getY())) {
                fruit.remove(i);
            }
        }
    }
    ...
    public void checkMove(Player player){
        ...
        int x = head.getX();
        int y = head.getY();
        if((!snakeIsAlive(player)) || (snakeCrashesIntoItself(head,player))){
        ...
        if(crashIntoAnotherSnake(head, player)){
